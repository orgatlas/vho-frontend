...
import {login as apiLogin, register as apiRegister, logout as apiLogout, getUser as apiGetUser} from 'src/services/api';
...
export const AuthProvider = ({children}: { children: ReactNode }) => {
    ...
    const register = async (first_name: string, last_name: string, email: string, password: string) => {
        setLoading(true);
        try {
            const data = await apiRegister(first_name, last_name, email, password);
            handleAuthResponse(data);
        } finally {
            setLoading(false);
        }
