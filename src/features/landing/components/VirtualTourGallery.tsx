...
import {getTourGallery} from 'src/services/api';
                ...
                >
                    <motion.div layout>
                        ...
                        <Box sx={{
                            ...
                            <Typography variant="caption" fontWeight={600}>{tour.title}</Typography>
                        </Box>
                    </motion.div>
    ...
    return (
        ...
        >
            ...
            >
                ...
                <Typography variant="h5" fontWeight={800} color="white" noWrap>
...
export const VirtualTourGallery = () => {
    ...
    useEffect(() => {
        const fetchTours = async () => {
            try {
                const data = await getTourGallery();
                if (data && data.length > 0) {
                    const mappedTours = data.map((t, i) => ({
                        id: `api-${i}`,
                        // Heuristic for title: Use Agent Name if available, or Company, or just "Virtual Tour"
                        title: t.agent ? `By ${t.agent}` : (t.company ? `By ${t.company}` : "Virtual Tour"),
                        location: t.address,
                        // Recycle mock images as placeholders since API doesn't return thumbnail
                        image: t.preview_url,
                        video: t.video_url
                    }));
    ...
    // Determine which list to use (API or Mock)
    const displayTours = tours.length > 0 ? tours : TOURS;

    // Construct the infinite list (memoized)
    const infiniteTours = useMemo(() => {
        // Ensure we have enough items to loop smoothly
        let items = [...displayTours];
        // If we have very few items, duplicate them more times
        while (items.length < 15) {
            items = [...items, ...displayTours];
        }
        return items;
    }, [displayTours]);
        ...
        <Box id="tour-gallery" sx={{
            ...
            <Container maxWidth="lg" sx={{textAlign: 'center', mb: {xs: 4, md: 8}, position: 'relative', zIndex: 10}}>
                <Typography variant="h2"
                            sx={{color: theme.palette.text.primary, fontWeight: 800, mb: 2, display: 'inline-flex'}}>
                    Properties We've Brought to&nbsp;
                </Typography>
                <Typography variant="h2"
                            sx={{color: theme.palette.primary.main, fontWeight: 800, mb: 2, display: 'inline-flex'}}>
                    Life.
                </Typography>
